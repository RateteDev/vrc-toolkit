...
export function MyCardModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  // The preview mirrors the DRAFT, not the saved profile: the point of this
  // modal is checking how the card will look to friends while editing it.
  const editing = phase === "loaded";
  const previewBio = editing ? form.bio : (profile?.bio ?? "");
  const previewLinks = editing
    ? form.bioLinks.map((u) => u.trim()).filter((u) => u.length > 0)
    : (profile?.bioLinks ?? []);
  const previewPronouns = editing ? form.pronouns.trim() : (profile?.pronouns ?? "");
  const statusLine = profile
    ? [previewPronouns, profile.statusDescription || profile.status].filter(Boolean).join(" · ")
    : "";
  ...
  return (
    <Modal ariaLabel="プロフィール編集" sheetClass="card-sheet" onRequestClose={requestClose}>
      ...
      <p className="card-bio" dangerouslySetInnerHTML={{ __html: renderMd(previewBio) }} />
      <BioLinkCards urls={previewLinks} />
