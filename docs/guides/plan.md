...
- ファイル名は ADR と同じ `YYYYMMDD-hhmm-{h1のタイトル}.md` とする。日時はプランを作成した日時を表し、h1 の文字制約（バッククォート・空白・`/` を含めない）も ADR ガイドに従う。
...
- `Goal` — このプランで何が動くようになるかを 1〜2 文で述べ、根拠 ADR への参照を置く。配下に h3 を `完了条件` → `前提` → `スコープ外` の順で置く。
    - `完了条件` — Goal を達成したと判定する条件。チェックボックスで、テスト・lint・grep など機械的に確認できる形で書く。コマンドを主語に期待結果まで直接書き、日本語の言い換えを重ねない。
...
- `Phase {番号}: {名称}` — 作業のまとまり。並行して実施する Phase は `2-1` / `2-2` のように枝番で表し、枝番も `{番号}` の一部として扱う。名称は対象が伝わる日本語で書き、パスやコマンドを括弧で添えない。配下に Step をチェックボックスで並べる。
...
{何が動くようになるか。根拠: `YYYYMMDD-hhmm-{ADRのタイトル}.md`}

### 完了条件

- [ ] {`コマンド` が {期待結果} で終了する、の形で書いた条件}
...
- 常体（〜である・〜とした）で書く。
- 同じ対象を日本語とコマンドで二重に書かない。日本語で伝わる記述は日本語だけで書き、対応するコマンド・パスを括弧で併記しない（NG: APIアプリのテスト（`just test apps/api`）が通る → 適切: API層のテストが通る）。この方針は Phase 名と mermaid のノードラベルにも適用する（NG: `apps/api への組み込み` → 適切: APIアプリへの組み込み）。
- コマンドの実行・確認が主目的の箇所（`検証`・`完了条件`）は逆に、コマンドを主語に期待結果まで直接書き、日本語の言い換えを重ねない（NG: テスト通過（`just test` で Failed が 0 件であることを確認する） → 適切: `just test` が Failed 0 件で終了する）。
- Step の本文には「何を作るか」だけを 1 文で書き、進め方や確認手順を本文に混ぜない。
- `検証` はエージェントがそのまま実行できるコマンドを主語に、期待結果まで直接書く。「動作確認する」のような抽象表現や、コマンドの日本語言い換えだけで済ませない。Step の `検証` は変更対象に閉じた確認を担い、全体の検収は `完了条件` が担う。
...
- ファイル名・関数名・コマンド名などのコード識別子は必ずインラインコードにする。ただし h1 はファイル名と一致させるため例外とし、識別子も装飾なしで書く。
...
- [ ] 日本語で伝わる記述（Phase 名・mermaid ラベル・地の文）にコマンド・パスを括弧で併記していない
- [ ] h1 を除き、コード識別子がすべてインラインコードになっている
- [ ] `完了条件` がコマンドを主語に期待結果まで直接書かれ、日本語の言い換え＋括弧の二重表現がない
