...
describe("geom", () => {
  ...
  it("clamps a center placed near an edge and updates it.ncx", () => {
    const it: CropItem = { natW: 1920, natH: 1080, zoom: 2, ncx: 0.0, ncy: 0.5 };
    const g = geom(it, A);
    // center was 0 but must be clamped to at least cw/2 -> ncx grows above 0.
    expect(it.ncx).toBeGreaterThan(0);
